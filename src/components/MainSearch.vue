<template>
  <div class="grey-wrapper flex justify-center items-center q-py-sm q-px-lg bg-white">
    <q-select
      borderless
      use-input
      v-model="reservation"
    ...
    >
      ...
      <LiteDateRangePicker v-on:selected="(newDates) => dateRangeChange(newDates)"/>
      <template v-slot:append>
        <q-icon v-if="dates.from" name="close" @click="dates.from = ''; dates.to = ''" class="cursor-pointer"/>
      </template>
    </q-input>
    <q-separator vertical inset />
    <q-input
      ...
      v-model="people"
    >
      <q-menu style="border: 1px solid #ccc; width: 175px;" class="no-shadow text-center q-pa-md">
        <div class="flex flex-column justify-center items-center">
          ...
          <div class="text-center q-pa-sm">
            ...
            <div v-for="(child, index) in children.people" :key="'child-' + index" class="q-pa-xs">
              <q-input
                outlined
                label="Wiek dziecka"
      ...
      <template v-slot:append>
        <q-icon v-if="people" name="close" @click="people = ''" class="cursor-pointer"/>
      </template>
    </q-input>
    <q-separator vertical inset />
    <q-input
    ...
    <div>
      <q-btn size="lg" unelevated color="primary" text-color="white" round icon="ti-search" class="q-mx-md"/>
    </div>
  </div>
</template>
...
const dateRangeChange = (newDates: { from: string; to: string }) => {
  ...
  daysCount.value = differenceInDays(new Date(newDates.to), new Date(newDates.from))
}
...
const savePeopleValues = () => {
  ...
  people.value = `Dorośli: ${adults.value.value}, Dzieci: ${children.value.value}, Wiek dzieci: ${children.value.people.join(', ')}`
}
</script>
